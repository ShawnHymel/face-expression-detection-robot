...
class WebcamInference:
    ...
    def _inference_loop(self):
        ...
        while self.running:
            ...
            # Actions: update web UI and animate the robot head
            self._animate_head()
            self._update_web_ui()
    ...
    def _animate_head(self):
        ...
        # Get bounding box info from largest box    
        box = self.result.boxes[largest_idx]
        class_name = self.result.class_names[largest_idx]
        confidence = self.result.scores[largest_idx]
        ...
        x_normalized = x_center / self.result.frame.shape[1]
        y_normalized = y_center / self.result.frame.shape[0]
        
        # Calculate box area for logging
        area = (box[2] - box[0]) * (box[3] - box[1])
        
        # Log what we would do
        print(f"\n[Servo Control] Tracking: {class_name}")
        print(f"  Box area: {area:.0f} px² (largest of {len(self.result.boxes)})")
